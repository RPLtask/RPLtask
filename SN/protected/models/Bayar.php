...
class Bayar extends CActiveRecord
{
	...
	public function rules()
	{
		...
		return array(
			array('id_penghuni, nilai_bayar, sisa_bayar,nilai_kontrak', 'numerical', 'integerOnly'=>true),
			array('status', 'length', 'max'=>10),
			array('tgl_dibayar, tgl_dibayar', 'date','format'=>'yyyy-M-d'),
			// The following rule is used by search().
			// Please remove those attributes that should not be searched.
			array('id_bayar, id_penghuni,  nilai_bayar, sisa_bayar, status, tgl_dibayar', 'safe', 'on'=>'search'),
		);
	}
